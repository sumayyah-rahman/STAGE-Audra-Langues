...
// ---------------------------------------------------------------------------
// Synthèse des cours déjà déclarés pour le mois
// ---------------------------------------------------------------------------
$dejaDeclares = [];
...
if ($profId !== '') {
    $sqlSynth = "
        SELECT
            ...
            SUM(
                DATEDIFF(
                    MINUTE,
                    CAST(heure_debut AS time),
                    CAST(heure_fin   AS time)
                )
            ) / 60.0 AS total_heures
        FROM dbo.AudraWeb_Saisie_Heures_Web
        WHERE id_prof = ?
          AND YEAR(date_seance) = ?
          AND MONTH(date_seance) = ?
    ...
    ";
    $stSynth = sqlsrv_query($conn, $sqlSynth, [$profId, (int)$annee, (int)$mois]);
} else {
    $sqlSynth = "
        SELECT
            id_cours,
        ...
        ORDER BY id_cours DESC
    ";
    $stSynth = sqlsrv_query($conn, $sqlSynth, [$PROF, (int)$annee, (int)$mois]);
}

if ($stSynth) {
    while ($rSynth = sqlsrv_fetch_array($stSynth, SQLSRV_FETCH_ASSOC)) {
        $dejaDeclares[] = [
            'id_cours'     => (int)($rSynth['id_cours'] ?? 0),
            'total_heures' => (float)($rSynth['total_heures'] ?? 0),
        ];
    }
    sqlsrv_free_stmt($stSynth);
}
...
<div class="panel">
  ...
  <div style="flex:1;text-align:center;font-size:24px;font-weight:800;color:#1d4ed8;letter-spacing:.3px;">
    ECRAN DE SAISIE DES COURS
  </div>
  ...
  <div style="margin:0 0 14px 0; padding:10px 12px; background:#fff7ed; border:1px solid #fdba74; border-radius:8px; color:#9a3412; font-size:14px; line-height:1.45;">
  Attention : si un ou plusieurs de vos cours n’apparaissent pas dans la liste, merci de contacter rapidement le bureau.
</div>
<p style="margin:0 0 14px 0; font-size:16px; color:#1d4ed8; font-weight:bold;">1) Sélectionner un cours</p>

<div id="blocDeclares" style="margin:10px 0 14px 0; padding:10px 12px; background:#f8fafc; border:1px solid #dbeafe; border-radius:8px; font-size:13px; line-height:1.45;">
  <div style="font-weight:bold; color:#1d4ed8; margin-bottom:6px; font-size:14px;">
    📘 Ci-dessous les Cours que vous avez déjà saisis à ce jour pour <?= htmlspecialchars($moisTxt) ?> <?= (int)$annee ?>
  </div>

  <?php if (!empty($dejaDeclares)): ?>
    <?php $totalDeclares = 0.0; ?>
    <div id="listeDeclares" style="line-height:1.55;">
      <?php foreach ($dejaDeclares as $dc): ?>
        <?php $totalDeclares += (float)$dc['total_heures']; ?>
        <div data-cours="<?= (int)$dc['id_cours'] ?>">
          N° cours <?= (int)$dc['id_cours'] ?> : <span class="heures-declarees"><?= number_format((float)$dc['total_heures'], 2, ',', ' ') ?></span> h
        </div>
      <?php endforeach; ?>
    </div>

    <div id="totalDeclaresBox" style="margin-top:8px; padding-top:6px; border-top:1px solid #dbeafe; font-weight:bold; color:#111827;">
      Total déclaré à ce jour : <span id="totalDeclaresValeur"><?= number_format((float)$totalDeclares, 2, ',', ' ') ?></span> h
    </div>

    <div style="margin-top:6px; color:#374151; font-size:12px;">
      Consultez le récapitulatif complet des cours à l’écran 3.
    </div>
  <?php else: ?>
    <div id="listeDeclares" style="color:#6b7280;">
      Aucun cours encore déclaré pour ce mois.
    </div>

    <div id="totalDeclaresBox" style="display:none; margin-top:8px; padding-top:6px; border-top:1px solid #dbeafe; font-weight:bold; color:#111827;">
      Total déclaré à ce jour : <span id="totalDeclaresValeur">0,00</span> h
    </div>
  <?php endif; ?>
</div>
...
function refreshBlocDeclares(){
  const liste = document.getElementById('listeDeclares');
  const totalBox = document.getElementById('totalDeclaresBox');
  const totalVal = document.getElementById('totalDeclaresValeur');
  if (!liste || !totalBox || !totalVal) return;

  const map = {};

  // 1) repartir de ce qui est déjà affiché dans l'encadré
  const lignesExistantes = liste.querySelectorAll('[data-cours]');
  lignesExistantes.forEach(el => {
    const id = String(el.getAttribute('data-cours') || '').trim();
    const span = el.querySelector('.heures-declarees');
    const txt = span ? span.textContent.trim().replace(',', '.') : '';
  ...
  // 2) écraser avec le cache si disponible
  Object.keys(seancesCache).forEach(idCours => {
    const seances = seancesCache[idCours] || [];
    let totalCours = 0;

    seances.forEach(s => {
      const d = parseFloat(s.duree || 0);
      if (!isNaN(d)) totalCours += d;
    });

    if (totalCours > 0) {
      map[String(idCours)] = totalCours;
    }
  });

  // 3) écraser avec le cours actuellement en mémoire (le plus récent)
  if (currentCours && Array.isArray(lignes)) {
    let totalCoursActif = 0;
    lignes.forEach(l => {
      const d = parseFloat(l.duree || 0);
      if (!isNaN(d)) totalCoursActif += d;
    });

    if (totalCoursActif > 0) {
      map[String(currentCours)] = totalCoursActif;
    } else {
      delete map[String(currentCours)];
    }
  }

  const ids = Object.keys(map).sort((a,b)=>parseInt(b,10)-parseInt(a,10));

  if (ids.length === 0) {
    liste.innerHTML = 'Aucun cours encore déclaré pour ce mois.';
  ...
  ids.forEach(id => {
    ...
    div.setAttribute('data-cours', id);
    div.innerHTML = `N° cours ${id} : <span class="heures-declarees">${h.toFixed(2).replace('.', ',')}</span> h`;
...
function getSelectedCoursDureeFaf(){
  const sel = document.getElementById('cours');
  if (!sel) return 0;
  const opt = sel.options[sel.selectedIndex];
  if (!opt) return 0;
  const n = parseFloat(opt.dataset.dureeFaf || '0');
  return isNaN(n) ? 0 : n;
}

function getSelectedCoursHeuresValideesEbp(){
  const sel = document.getElementById('cours');
  if (!sel) return 0;
  const opt = sel.options[sel.selectedIndex];
  if (!opt) return 0;
  const n = parseFloat(opt.dataset.heuresValideesEbp || '0');
  return isNaN(n) ? 0 : n;
}

function getSelectedCoursDateAtteinteEbp(){
  const sel = document.getElementById('cours');
  if (!sel) return '';
  const opt = sel.options[sel.selectedIndex];
  if (!opt) return '';
  return String(opt.dataset.dateAtteinteEbp || '').trim();
}

function getCurrentCoursTotalHeures(){
  let total = 0;
...
function getFafReachedInfo(dureeFaf){
  const seuil = parseFloat(dureeFaf || 0);
  if (!seuil || seuil <= 0 || !Array.isArray(lignes) || lignes.length === 0) {
    return { reached: false, date: '', total: 0 };
  }

  const sorted = lignes.slice().sort((a, b) => {
    const af = (a.date || '').split('/').reverse().join('-') + ' ' + (a.debut || '00:00');
    const bf = (b.date || '').split('/').reverse().join('-') + ' ' + (b.debut || '00:00');
  ...
  try {
    ...
    if (js && js.success) {
      ...
      l.etat  = 'lock';
	        ...
	        // Enregistrer aussi mode + élève absent dans AudraWeb_Seance_Extras
      try {
        const fdExtra = new FormData();
        fdExtra.append('id_seance', String(l.id));
        fdExtra.append('mode', String(l.mode || 'PRESENTIEL'));
        fdExtra.append('faf', '0');
        fdExtra.append('km', '0');
        fdExtra.append('eleve_absent', l.eleve_absent ? '1' : '0');

        await fetch(AUDRA_ACTIONS + '/save_seance_extras.php', {
          method: 'POST',
          body: fdExtra,
          credentials: 'include',
          cache: 'no-store'
        ...
        console.warn('save_seance_extras failed', e);
      }
...
const absInput = document.getElementById('abs_'+id);
const modeInput = document.querySelector(`input[name="mode_${id}"]:checked`);
  ...
  if(t2<=t1){
   ...
   const nouvelleDuree = (t2 - t1) / 60;
  const dureeFaf = getSelectedCoursDureeFaf();
  const heuresValideesEbp = getSelectedCoursHeuresValideesEbp();
  const dateAtteinteEbp = getSelectedCoursDateAtteinteEbp();
  const totalActuelSansCetteLigne = getCurrentCoursTotalHeures() - (parseFloat(l.duree || 0) || 0);
  const totalReferenceAvant = totalActuelSansCetteLigne > 0 ? totalActuelSansCetteLigne : heuresValideesEbp;
  const nouveauTotal = totalReferenceAvant + nouvelleDuree;
  const fafInfo = getFafReachedInfo(dureeFaf);
  
  console.log('DEBUG FAF', {
  currentCours,
  dureeFaf,
  heuresValideesEbp,
  dateAtteinteEbp,
  totalActuelSansCetteLigne,
  totalReferenceAvant,
  nouvelleDuree,
  nouveauTotal,
  ...
  if (dureeFaf > 0) {
    const resteAvant = dureeFaf - totalReferenceAvant;

       if (totalReferenceAvant >= dureeFaf) {
      const dateTexte = dateAtteinteEbp || fafInfo.date || '';
      const msg = "Attention : la durée prévue du cours n°" + currentCours +
        (dateTexte ? " a été atteinte le " + dateTexte : "") +
        ". Si vous saisissez cette nouvelle séance, la durée sera dépassée." +
        "\n\nVérifiez votre feuille de présence ou interrogez le bureau." +
        "\n\nVoulez-vous vraiment continuer ?";
      if (!confirm(msg)) {
        l.etat='edit'; rafraichirTableau(); return;
      }
      } else if (nouveauTotal > dureeFaf) {
      const msg = "Attention : la durée prévue du cours n°" + currentCours +
        " est déjà atteinte ou va être dépassée par cette nouvelle séance." +
        "\nIl restait " + resteAvant.toFixed(2).replace('.', ',') + " h avant d’atteindre la limite du contrat." +
        "\n\nVérifiez votre feuille de présence ou interrogez le bureau." +
        "\n\nVoulez-vous vraiment continuer ?";
      if (!confirm(msg)) {
        l.etat='edit'; rafraichirTableau(); return;
      }
    } else if (resteAvant <= 0.5) {
      const msgBox = document.getElementById('msg');
      if (msgBox) {
        msgBox.textContent = "⚠️ Attention : il ne reste plus que " + resteAvant.toFixed(2).replace('.', ',') +
          " h avant d’atteindre la durée prévue du cours n°" + currentCours + ".";
      }
    }
  }

  l.date = formatDateFr(d);
  l.debut = hd;
  l.fin = hf;
  l.duree = nouvelleDuree;
  l.eleve_absent = !!(absInput && absInput.checked);
  l.mode = modeInput ? modeInput.value : 'PRESENTIEL';
  l.etat = 'lock';
  saveSeance(l);
}
...
function rafraichirTableau(){
  ...
  tr.innerHTML=`
    ...
    <td>${l.eleve_absent ? 'OUI' : 'NON'}</td>
    <td>${(l.mode || 'PRESENTIEL').toUpperCase()}</td>
    <td>🔒</td>`;
} else {
  if(l.etat==='edit'){
    tr.innerHTML=`
...
<td style="white-space:nowrap;">
  <label style="font-size:12px;">
    <input type="checkbox" id="abs_${l.id}" ${l.eleve_absent ? 'checked' : ''}> Oui
  </label>
</td>
<td style="white-space:nowrap;">
  <label style="font-size:12px; margin-right:8px;">
    <input type="radio" name="mode_${l.id}" value="PRESENTIEL" ${(!l.mode || l.mode === 'PRESENTIEL') ? 'checked' : ''}> Présentiel
  </label>
  <label style="font-size:12px;">
    <input type="radio" name="mode_${l.id}" value="VISIO" ${(l.mode === 'VISIO') ? 'checked' : ''}> Visio
  </label>
</td>
<td>
...
const res = await fetch(url, {
  ...
  console.error("Réponse HTTP non OK pour select_cours.php :", errTxt);

  const msgBox = document.getElementById("msg");
  if (msgBox) {
    msgBox.textContent = "⚠️ Impossible de charger la liste des cours (code " + res.status + "). Voir console.";
  }
  return;
}

    const txt = await res.text();
console.log("Réponse brute select_cours:", txt);

let js = null;
try {
  js = JSON.parse(txt);
} catch(e) {
  console.error("JSON invalide select_cours.php :", txt);
  const msgBox = document.getElementById("msg");
  if (msgBox) {
    msgBox.textContent = "⚠️ Réponse invalide de select_cours.php. Voir console navigateur.";
  }
  return;
}
  ...
  opt.dataset.dureeFaf = String(c.duree_faf || 0);
  opt.dataset.heuresValideesEbp = String(c.heures_validees_ebp || 0);
  opt.dataset.dateAtteinteEbp = String(c.date_atteinte_ebp || '');
  sel.appendChild(opt);
});
...
async function chargerSeances(idCours) {
  ...
  if (seancesCache[idCours]) {
    lignes = seancesCache[idCours].map(s => ({
      ...
      eleve_absent: (parseInt(s.eleve_absent || 0, 10) === 1),
      etat: 'lock'
    }));
    rafraichirTableau();
    return;
...
console.log('Réponse brute list_saisies pour cours ' + idCours + ' :', txt);

let js = null;
try {
  js = JSON.parse(txt);
} catch(e) {
  console.error('JSON invalide list_saisies.php :', txt);
  lignes = [];
  rafraichirTableau();
  return;
}
