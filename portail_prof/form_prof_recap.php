...
if ($profId !== '') {
  $sqlHeures = "
    SELECT
      s.id_cours,
      SUM(CAST(s.duree AS FLOAT)) AS heures,
      CASE
        WHEN COUNT(DISTINCT UPPER(COALESCE(x.mode, 'PRESENTIEL'))) <= 1
          THEN MAX(UPPER(COALESCE(x.mode, 'PRESENTIEL')))
        ELSE 'MIXTE'
      END AS mode_label
...
ISNULL(x.eleve_absent, 0) AS eleve_absent
    FROM dbo.AudraWeb_Saisie_Heures_Web s
  ...
  while ($r = sqlsrv_fetch_array($stDetail, SQLSRV_FETCH_ASSOC)) {
    ...
    $seancesDetail[] = [
      ...
      'mode'         => (string)($r['mode'] ?? 'PRESENTIEL'),
		'km'           => $kmRow,
		'eleve_absent' => (int)($r['eleve_absent'] ?? 0),
    ];
  }
...
<div class="container" style="padding:20px">
  ...
  <h1>ÉCRAN DE RÉCAPITULATIF DES SAISIES ET CORRECTIONS</h1>
<p style="margin:0 0 14px 0; font-size:16px; color:#1d4ed8; font-weight:bold;">Récapitulatif des saisies</p>

  <table>
    <thead>
      <tr><th>Cours</th><th>Heures</th><th>Visio / Présentiel</th></tr>
    </thead>
    <tbody>
      <?php if (empty($cours)): ?>
        <tr><td colspan="3">Aucune saisie enregistrée pour ce mois.</td></tr>
      <?php else: ?>
        <?php foreach ($cours as $c): ?>
          <tr>
  <td><?= htmlspecialchars((string)$c['id_cours']) ?></td>
  <td><?= number_format((float)($c['heures'] ?? 0), 1, ',', ' ') ?></td>
  <td><?= htmlspecialchars((string)($c['mode_label'] ?? 'PRESENTIEL')) ?></td>
</tr>
        <?php endforeach; ?>
      <?php endif; ?>
    </tbody>
  ...
  <h2 style="margin-top:24px">Vue globale des séances (tous les cours) -  Vous pouvez encore modifier vos cours directement dans le tableau ci dessous</h2>
  ...
  <table id="table-detail" class="table-detail">
    <caption>N° cours — Date — Horaire — Durée — Absence élève — Mode — KM — Actions</caption>
    <thead>
      <tr>
  <th>N° cours</th>
  <th>Date</th>
  ...
  <input type="checkbox"
    ...
    <tfoot>
      <tr>
        <td colspan="5" style="text-align:right;font-weight:bold">Total heures (détail)</td>
        <td id="hours-detail-total" style="font-weight:bold"><?= number_format((float)$totalDetailHeures, 2, ',', ' ') ?></td>
        <td></td>
      </tr>
      <tr>
        <td colspan="5" style="text-align:right;font-weight:bold">Total KM (séances)</td>
...
fd2.append('eleve_absent', inpAbs.checked ? '1' : '0');
...
document.addEventListener('input', function(ev){
  ...
  if (
    ...
    t.classList.contains('se-debut') ||
    t.classList.contains('se-fin') ||
    t.classList.contains('se-extra-abs')
  ) {
    markRecapDirty();
  }
}, true);
...
if (t.classList.contains('se-extra-abs')) {
  const id = t.getAttribute('data-id');
  markRecapDirty();
  saveRowAuto(id);
  return;
