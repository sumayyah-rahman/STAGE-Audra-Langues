...
<div id="cours_fields_wrap" style="display:none; margin-top:12px;">
 <div>
    <label for="rp_absences"><b>1. Heures d’absence élève sur ce cours</b></label>
    <input type="number" id="rp_absences" name="rp_absences" min="0" step="0.5"
  placeholder="0"
  style="width:220px; padding:8px; border:1px solid #ccc; border-radius:6px;">
    <div style="font-size:12px; color:#6b7280; margin-top:6px;">
      Indiquez ici le nombre d’heures déclarées alors que l’élève ne s’est pas présenté.
    </div>
  </div>
  <div>
    <label for="rp_remontee"><b>2. Remontée pédagogique</b></label>
  ...
  <div style="margin-top:12px;">
    <label for="rp_infos"><b>3. Infos / Problème (cours ou élève)</b></label>
  ...
  SELECT id_cours, remontee, infos, heures_absence, updated_at
  FROM dbo.AudraWeb_Remarques_ParCours
  WHERE id_prof=? AND annee=? AND mois=?
  ORDER BY id_cours
", [$rpProf, (int)$annee, (int)$mois]);
	...
	<?php
$hasAbsenceRenseignee = false;
foreach ($rpRows as $__rr) {
  $v = (float)($__rr['heures_absence'] ?? 0);
  if ($v > 0) {
    $hasAbsenceRenseignee = true;
    break;
  }
}
?>
  ...
  <th style="border:1px solid #e5e7eb; padding:6px 8px; text-align:left;">Absences</th>
...
(function(){
  ...
  form.addEventListener('submit', async function(ev){
    ev.preventDefault();
	const absEl = document.getElementById('rp_absences');
	const absVal = absEl ? String(absEl.value || '').trim() : '';
	const hasAbsenceRenseignee = <?= !empty($hasAbsenceRenseignee) ? 'true' : 'false' ?>;

if (!hasAbsenceRenseignee && (absVal === '' || absVal === '0' || absVal === '0.0' || absVal === '0,0')) {
  const okAbs = confirm(
    "Rappel important : si vous avez déclaré des heures de cours alors que l'élève ne s'est pas présenté, vous devez indiquer ces heures dans le bloc 1 « Heures d’absence élève sur ce cours ».\n\n" +
    "Confirmez-vous qu’il n’y a aucune heure d’absence élève à signaler avant de passer au récapitulatif et aux uploads ?"
  );
  if (!okAbs) {
    if (absEl) absEl.focus();
    return;
  }
}
    const fd = new FormData(form);
...
if (wrap) {
  ...
  const absEl = document.getElementById('rp_absences');
if (absEl) {
  let v = String(js.heures_absence || "0").trim();
  if (v === '' || v === ',00' || v === '.00') v = '0';
  v = v.replace(',', '.');
  absEl.value = v;
}
...
window.audraMetaLoadForCours = loadForCours;
  ...
  btn.addEventListener('click', async function(){
    ...
    if (absVal === '' || absVal === '0' || absVal === '0.0' || absVal === '0,0') {
      const okAbs = confirm(
        "Avez-vous déclaré des heures de cours même si l'élève ne s'est pas présenté ?\n\n" +
        "Si oui, indiquez le nombre d'heures d'absence dans le bloc 1 avant d'enregistrer."
      );
      if (!okAbs) {
        if (absEl) absEl.focus();
        return;
    ...
    fd.append('heures_absence', absEl ? (absEl.value || "0") : "0");
